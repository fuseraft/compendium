...
<#
.SYNOPSIS
    Compendium CLI installer for Windows.

.DESCRIPTION
    Downloads the latest compendium CLI release from GitHub and installs it to
    %LOCALAPPDATA%\compendium\bin, then adds that directory to the user PATH.
...
.EXAMPLE
    ...
    .\install.ps1 -InstallDir "C:\Tools\compendium"
#>
param(
    [string] $InstallDir = "$env:LOCALAPPDATA\compendium\bin"
)
...
$Headers = @{ 'User-Agent' = 'compendium-installer' }
...
$Archive = "compendium-cli-$Version-$RID.zip"
...
$TargetExe   = Join-Path $InstallDir 'compendium.exe'

if (Test-Path $TargetExe) {
    Write-Host "Updating existing installation to compendium $Version ($RID)..."
} else {
    Write-Host "Installing compendium $Version ($RID)..."
}

###############################################################################
# Download, extract, install
###############################################################################
$TmpDir = Join-Path $env:TEMP "compendium-install-$([System.IO.Path]::GetRandomFileName())"
...
try {
    ...
    $ExeSource = Get-ChildItem -Path $TmpDir -Recurse -Filter 'compendium.exe' |
        Select-Object -First 1 -ExpandProperty FullName

    if (-not $ExeSource) {
        Write-Error "compendium.exe not found in archive. The release may be malformed."
        exit 1
    }
    ...
    Write-Host ""
    Write-Host "  compendium $Version installed to $TargetExe"
} finally {
...
if ($PathEntries -contains $NormalizedInstallDir) {
    ...
    Write-Host "  Restart your terminal (or open a new one) to use compendium."
}

Write-Host ""
Write-Host "  Run 'compendium --version' to verify the installation."
Write-Host ""
