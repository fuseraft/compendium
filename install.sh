#!/usr/bin/env bash
# Compendium CLI installer — Linux and macOS
#
# Usage:
...
VERSION="${TAG#v}"
ARCHIVE="compendium-cli-${VERSION}-${RID}.tar.gz"
...
if command -v "$BINARY" >/dev/null 2>&1; then
  ...
  echo "Updating${CURRENT_VERSION:+ from ${CURRENT_VERSION}} to compendium ${VERSION} (${RID})..."
else
  echo "Installing compendium ${VERSION} (${RID})..."
fi

###############################################################################
# Download and extract
###############################################################################
TMP_DIR="$(mktemp -d 2>/dev/null || mktemp -d -t compendium)"
...
echo ""
echo "  compendium ${VERSION} installed to ${INSTALL_DIR}/${BINARY}"
...
echo "  Run 'compendium --version' to verify the installation."
echo ""
